// The daemon is windowless, so stderr goes nowhere the user can see.
// Redirect it into a log file in the config dir at startup; all existing
// eprintln calls keep working and become debuggable after the fact. The
// UI crate carries its own copy of this file, the two must agree on the
// log path.
...
pub fn log_path() -> Option<PathBuf> {
    Some(crate::config_dir()?.join("toolshot.log"))
}
